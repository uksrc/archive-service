...
2. Make sure the images created are added to minikube's docker repository and not to the host machine:
    ...
    ```
3. clean and build the archive-service
    ```
    gradle clean
    gradle build -x test
    ```
4. Expose the archive service on the minikube cluster and the displayed localhost:<port> should resolve the service (just add /tap)
    ```
